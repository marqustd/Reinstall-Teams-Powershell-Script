...
# Install Teams by Marek Zabialowicz
...
$install= Read-Host "Do you want to install Teams again (Y/N)?"
...
if ($install -eq "Y"){
    ...
    $downloadLink = "https://go.microsoft.com/fwlink/p/?LinkID=869426&culture=en-us&country=WW&lm=deeplink&lmsrc=groupChatMarketingPageWeb&cmpid=directDownloadWin64"
    $teamsInstaller = "Teams_windows_x64.exe"

    #check if downloadFolder contains Teams_windows_x64.exe
    if (Test-Path "$downloadFolder\$($teamsInstaller)"){
        Write-Host "Teams Installer Found" -ForegroundColor Green
    }
    else{
        Write-Host "Downloading Teams Installer" -ForegroundColor Yellow
        Invoke-WebRequest "$downloadLink" -outfile "$downloadFolder\$($teamsInstaller)"
        Write-Host "Teams Installer Downloaded" -ForegroundColor Green
    }

    Write-Host "Running Teams installator..." -ForegroundColor Yellow
    Invoke-Expression -Command "$downloadFolder\$($teamsInstaller)"
}